...
I'm being a bit disingenuous when I state that state management is easy, because really (with a few exceptions) it's not _really_ any particular state management library that makes state management difficult, it's really the act of designing state structure and data flows that are difficult. What's more, with state management frameworks that are _reactive_ in nature, such as `mobx`, `vuex`, `rxjs` and the like, it can be very difficult to know _where_ a state change has taken place. In addition, it's all to easy to make a mistake when updating state where the wrong data is assigned to the wrong key etc.
...
Cool, so let's get in to creating a super simple and lightweight state management library that's more than capable of managing big app state! Listing out the requirements we have:
...
I probably won't be able to do a great job of explaining how the nitty gritty of `RemoveFirstFromTuple` works, but essentially what we're doing is plucking out the arguments of the mutator's implementation function type, denoted as `Fn` as a tuple of types, and removing the first type from the tuple to be left with a resultant tuple of the remaining arguments. We then pass this resultant type in as the type of `...args` to complete the inference. Neat! That wraps up changing data.

## Subscribing to data changes

Another relatively simple one is implementing pub/sub functionality. Again, another simple variable is needed to keep track of subscriptions to state changes
...
And then unsubscribing (so we don't end up with memory leaks!):
...
Now all that's left is to notify the subscribers whenever data changes, for that, we'll add a little hook in to the calling of a mutator to map over the subscribers and call them one-by-one:
