...
I focused on ensuring that the usage of the `useForm` hook is _fully_ type safe. And I don't mean simply that `useForm` returns type safe functions and values (i.e `<TextField />` only supports managing strings, so it's only allowed to register keys using `registerTextInput()` from the schema whose values _are_ strings).

I'm a strong believer that leveraging type systems to this extent greatly reduced the number of bugs and inconsistencies in a code base, as well as serving as a form of documentation for developers.
