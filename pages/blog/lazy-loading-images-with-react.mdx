...
3. Creates a React component that takes in `props` that extends the default `<img />` tag props and do not include the `src` attribute on the `<img />` tag
...
The last piece in the puzzle is to connect the React component to the observer.

- Obtain a reference to the observer when the component mounts
- During render, obtain DOM node references to the wrapping and image elements
