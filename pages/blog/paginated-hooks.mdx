...
> The reason that fetchInitial, fetchNextPage and refresh do not have any arguments is because these functions are inferred from the ReturnType of the passed fetcher function.

The last thing to map out is what the fetcher function looks like. The `usePaginatedFetcher` hook takes in a function as the first argument that is responsible for actually fetching the data from somewhere. This function receives `from` and `size` as the first two arguments, to which the function is expected to return another function that performs the fetching. The reason the fetcher is structured as a higher-order function is because the hook is responsible for managing the state of `from` and `size`, but there may be additional parameters passed in to perform the fetching of the data, like ids, filters, searches, sorts, etc... Here's the definition of the fetcher function in Typescript:
...
The second thing to note is that there's a variable I haven't introduced yet, `idKeyName`. As mentioned above, this paginated fetcher hook can't know about the structure of the underlying data, yet it keeps track of the list of results to ve displayed in the UI. These two things are contradictory of course, so there needs to be some way for the hook to know how to uniquely identify a particular result. For this, a `idKeyName` is passed in as a new argument to the hook and is used to pull out a unique identifier per record. This unique identifier is set in the state of the hook and passed back out to the component using the hook to map over and render the results:
...
However, we want the methods exposed by the hook to fetch the data to look like this:
